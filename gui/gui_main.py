...
class Gui:
    def __init__(self,mydata,gui_core,window,canvas):
        ...
        self.root.option_add("*TCombobox*Listbox*Font", ("Consolas",15))
        self.br = ""
        self.tl = ""
    ...
    def callback2(self,event):
        ...
        if x>=(1283/1600)*(self.scr_width-20) and x<=(1391/1600)*(self.scr_width-20) and y >= (718/900)*(self.scr_height-60) and y<= (790/900)*(self.scr_height-60):
            ...
            self.bout = self.canvas2.create_rectangle(
                (1283/1600)*(self.scr_width-20),(718/900)*(self.scr_height-60), 
                (1391/1600)*(self.scr_width-20),(790/900)*(self.scr_height-60),
                outline="#5442f5",
                fill="green",
                width=0,
                tags=("booked")
    ...
    def showcart(self):
        ...
        self.h=h
        self.canvas3 = Canvas(
            self.root, width = self.root.winfo_screenwidth()-20,height = self.scr_height-60,
            relief='flat',highlightthickness=0,scrollregion=(0,0,700,h+300)
        )
    ...
    def setcart(self):
        ...
        qtylabel = Label(columnhead,text="Quantity",font=("Liberation Serif",20))
        pricelabel = Label(columnhead,text="Price",font=("Liberation Serif", 20))
        ...
        for i in range(len(self.cart)):
            ...
            w = Spinbox(
                menubar, bg="white",fg="black",font=("Consolas",30),
                width=1,relief="flat",from_=1,bd=3,to=9,wrap=True,
                buttondownrelief="flat",buttonuprelief="flat",
                textvariable=self.var,state="readonly"
            )        
        ...
        if len(self.cart)!=0:
            self.confirmbtn = Button(self.root,text="Confirm purchase",bg="#56f545",font=("Liberation Serif",20),relief="flat",command=lambda:self.purchase(y))
    ...
    def purchase(self,y):
        ...
        self.curpmptopt.trace('w', self.OptionCallBack)
        
        self.pymtopts = Combobox(
            self.root,font=("Consolas",18),textvariable=self.curpmptopt,
            values=('Cash on delivery','Credit/Debit card'),state="readonly"
        )   
        #pymtopts.current(0)
    ...
    def showbookdetails(self,book):
        ...
        self.canvas2 = Canvas(
            self.root, width = self.root.winfo_screenwidth()-20,height = self.scr_height-60,
            relief='flat',highlightthickness=0,scrollregion=(0,0,700,(self.scr_height-60)*1.2)
        )
        self.vbar.grid(row=1, column=1,sticky="NSEW")
        ...
        self.w = Spinbox(
            self.root, bg="white",fg="black",font=("Consolas",35),width=1,relief="flat",
            from_=1,to=9,bd=0,wrap=True,buttondownrelief="flat",buttonuprelief="flat",
            textvariable=self.var,state="readonly",
        )
        ...
        if y:
            ...
            self.w["state"] = "disabled"
            self.pricelbl = self.canvas2.create_text(
                self.scr_width*0.38, self.scr_height*0.72, anchor="nw",
                text=f"Rs.{int(self.data1['price'])*int(k[2])}",
                font=("Liberation Serif",50),tags=("price")
            )
            self.bout = self.canvas2.create_rectangle(
                (1283/1600)*(self.scr_width-20),(718/900)*(self.scr_height-60), 
                (1391/1600)*(self.scr_width-20),(790/900)*(self.scr_height-60),
            ...
            self.pricelbl = self.canvas2.create_text(
                self.scr_width*0.38, self.scr_height*0.72, anchor="nw",
                text=f"Rs.{self.data1['price']}",font=("Liberation Serif",50),tags=("price")
            )
    ...
    def editprice(self):
        ...
        self.pricelbl = self.canvas2.create_text(
            self.scr_width*0.38, self.scr_height*0.72, anchor="nw",
            text=f"Rs.{price}",font=("Liberation Serif",50),tags=("price")
        )
        ...
        for i in l:
            ...
            if len(l)>4:
                if i==s+3:
                    ...
                    ynum += self.booky*1.5
        self.menubar.lift()
        self.pgstatus = self.canvas1.create_text(
            self.scr_width-125,1275,text=f"showing page {self.pgno+1} of 3",fill="white", 
            font=("Consolas",13),tags=("todel")
        )
    ...
    def search(self,*h):
        ...
        self.rarc = self.canvas1.create_arc(
            br[0]-16,tl[1]+1,br[0]+16,br[1]-1,start=270,extent=180,
            fill="white",width=0,style=CHORD,outline="white",tags="todel2"
        )
